  ...
  return (
    <>
      ...
      {data?.applicationList?.map((application, i) => (
        <div
          key={i}
          className="bg-gradient-to-r from-[#52C3FF] to-[#D0EAFF] p-5 flex flex-col md:flex-row justify-around rounded-xl shadow-xl relative mb-3"
        >
          <img
          ...
          <div className="p-5 flex flex-col w-full text-white">
            ...
            <h1 className="font-bold">
              Student ID :{" "}
              <span className="font-normal">{application?.studentId}</span>
            </h1>
            <h1 className="font-bold">
              Country :{" "}
              <span className="font-normal">{application?.country}</span>
            </h1>
          </div>
          <div className="p-5 w-full flex flex-col md:flex-row justify-end gap-3">
            {user?.role === "admin" ? (
